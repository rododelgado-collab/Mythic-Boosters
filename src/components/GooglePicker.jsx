...
const MOCK_ACCOUNTS = [
  {
    name: 'Diego Cárdenas',
    email: 'diego.cardenas@gmail.com',
    initials: 'DC',
    color: '#4285f4',
  },
  {
    name: 'Diego C.',
    email: 'd.cardenas.mtg@gmail.com',
    initials: 'DC',
    color: '#34a853',
  },
]

export function GooglePicker({ onSelect, onClose }) {
  return (
    <div
      ...
      >
        ...
        <div className="px-6 pt-6 pb-4 flex items-start justify-between">
          <div className="flex flex-col gap-1">
            {/* Logo Google SVG inline simplificado */}
            ...
            <p className="text-[13px] text-[#202124] font-medium mt-3">Iniciar sesión</p>
            <p className="text-[12px] text-[#5f6368]">para continuar en Mythic Boosters</p>
          </div>
          <button
            onClick={onClose}
            className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-gray-100 transition-colors text-[#5f6368]"
          >
            <X size={18} />
          </button>
        </div>
          ...
          {MOCK_ACCOUNTS.map((account) => (
            ...
            >
              ...
              <div className="flex flex-col min-w-0">
                <span className="text-[14px] text-[#202124] font-medium truncate">{account.name}</span>
                <span className="text-[12px] text-[#5f6368] truncate">{account.email}</span>
              </div>
            </button>
          ))}
        </div>
        ...
        <div className="px-4 py-3 flex items-center justify-between">
          <span className="text-[11px] text-[#5f6368]">Política de privacidad · Términos</span>
        </div>
      </div>
    </div>
  )
